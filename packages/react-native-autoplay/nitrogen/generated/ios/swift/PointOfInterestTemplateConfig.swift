///
/// PointOfInterestTemplateConfig.swift
...
/**
 * Represents an instance of `PointOfInterestTemplateConfig`, backed by a C++ struct.
 */
public typealias PointOfInterestTemplateConfig = margelo.nitro.swe.iternio.reactnativeautoplay.PointOfInterestTemplateConfig

public extension PointOfInterestTemplateConfig {
  private typealias bridge = margelo.nitro.swe.iternio.reactnativeautoplay.bridge.swift

  /**
   * Create a new instance of `PointOfInterestTemplateConfig`.
   */
  init(id: String, onWillAppear: ((_ animated: Bool?) -> Void)?, onWillDisappear: ((_ animated: Bool?) -> Void)?, onDidAppear: ((_ animated: Bool?) -> Void)?, onDidDisappear: ((_ animated: Bool?) -> Void)?, onPopped: (() -> Void)?, autoDismissMs: Double?, title: AutoText, items: [PointOfInterest], actionStrip: NitroActionStrip?, colors: NitroPointOfInterestColors?, onSelectItem: ((_ itemId: String) -> Void)?) {
    ...
    }(), title, { () -> bridge.std__vector_PointOfInterest_ in
      var __vector = bridge.create_std__vector_PointOfInterest_(items.count)
      for __item in items {
        __vector.push_back(__item)
      }
      return __vector
    }(), { () -> bridge.std__optional_NitroActionStrip_ in
      if let __unwrappedValue = actionStrip {
        return bridge.create_std__optional_NitroActionStrip_(__unwrappedValue)
      } else {
        return .init()
      }
    }(), { () -> bridge.std__optional_NitroPointOfInterestColors_ in
      if let __unwrappedValue = colors {
        return bridge.create_std__optional_NitroPointOfInterestColors_(__unwrappedValue)
      } else {
        return .init()
      }
    }(), { () -> bridge.std__optional_std__function_void_const_std__string_____itemId______ in
      if let __unwrappedValue = onSelectItem {
        return bridge.create_std__optional_std__function_void_const_std__string_____itemId______({ () -> bridge.Func_void_std__string in
  ...
  @inline(__always)
  var actionStrip: NitroActionStrip? {
    return self.__actionStrip.value
  }
  
  @inline(__always)
  var colors: NitroPointOfInterestColors? {
    return self.__colors.value
  }
  
  @inline(__always)
  var onSelectItem: ((_ itemId: String) -> Void)? {
    return { () -> ((_ itemId: String) -> Void)? in
      if bridge.has_value_std__optional_std__function_void_const_std__string_____itemId______(self.__onSelectItem) {
        let __unwrapped = bridge.get_std__optional_std__function_void_const_std__string_____itemId______(self.__onSelectItem)
        return { () -> (String) -> Void in
          let __wrappedFunction = bridge.wrap_Func_void_std__string(__unwrapped)
          return { (__itemId: String) -> Void in
            __wrappedFunction.call(std.string(__itemId))
          }
        }()
      } else {
        return nil
