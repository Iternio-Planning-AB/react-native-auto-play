...
public extension MapTemplateConfig {
  ...
  init(id: String, onWillAppear: ((_ animated: Bool?) -> Void)?, onWillDisappear: ((_ animated: Bool?) -> Void)?, onDidAppear: ((_ animated: Bool?) -> Void)?, onDidDisappear: ((_ animated: Bool?) -> Void)?, onPopped: (() -> Void)?, autoDismissMs: Double?, visibleTravelEstimate: VisibleTravelEstimate?, onDidPan: ((_ translation: Point, _ velocity: Point?) -> Void)?, onDidUpdateZoomGestureWithCenter: ((_ center: Point, _ scale: Double) -> Void)?, onClick: ((_ center: Point) -> Void)?, onDoubleClick: ((_ center: Point) -> Void)?, onAppearanceDidChange: ((_ colorScheme: ColorScheme) -> Void)?, onStopNavigation: @escaping () -> Void, onAutoDriveEnabled: (() -> Void)?, mapButtons: [NitroMapButton]?, headerActions: [NitroAction]?, defaultGuidanceBackgroundColor: NitroColor?, keepPanButtonWhilePanning: Bool?, panButtonScrollPercentage: Double?, onDidChangePanningInterface: ((_ isPanningInterfaceVisible: Bool) -> Void)?) {
    self.init(std.string(id), { () -> bridge.std__optional_std__function_void_std__optional_bool_____animated______ in
      ...
      if let __unwrappedValue = keepPanButtonWhilePanning {
  ...
  @inline(__always)
  var keepPanButtonWhilePanning: Bool? {
    return { () -> Bool? in
      if bridge.has_value_std__optional_bool_(self.__keepPanButtonWhilePanning) {
        let __unwrapped = bridge.get_std__optional_bool_(self.__keepPanButtonWhilePanning)
        return __unwrapped
      } else {
        return nil
      }
