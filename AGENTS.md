...
- Both `NSLocking.withLock { }` and explicit `lock()` / `defer { unlock() }` are fine, and both appear in the codebase (`VoiceInputManager` uses the former, the newer locks the latter). `withLock` *looks* like it needs iOS 16, but Foundation declares it `@available(iOS 8.0, *)` with `@_alwaysEmitIntoClient`, so it is inlined into the client and back-deploys below the pod's 15.1 floor — it has been flagged as an availability problem in review before, wrongly. Don't rewrite one form into the other.
